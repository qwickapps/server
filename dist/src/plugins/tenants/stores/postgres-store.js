 ...
 * ```
 */
export function postgresTenantStore(config) {
    const { pool: poolOrFn, tenantsTable = 'tenants', membershipsTable = 'tenant_memberships', schema = 'public', autoCreateTables = true, } = config;
    ...
    return {
        ...
        `);
                ...
                throw new Error(`Failed to initialize tenants tables: ${error instanceof Error ? error.message : String(error)}`);
            }
        },
        async getById(id) {
        ...
        async create(input) {
            ...
            const result = await getPool().query(`INSERT INTO ${tenantsTableFull} (name, type, owner_id, metadata)
         VALUES ($1, $2, $3, $4)
         RETURNING *`, [name, type, owner_id, JSON.stringify(metadata)]);
        ...
        async update(id, input) {
            ...
            values.push(id);
            const result = await getPool().query(`UPDATE ${tenantsTableFull}
         SET ${updates.join(', ')}
         WHERE id = $${paramCount}
         RETURNING *`, values);
            return result.rows[0] || null;
        ...
        async search(params) {
            ...
            // Get total count
            const countResult = await getPool().query(`SELECT COUNT(*) FROM ${tenantsTableFull} ${whereClause}`, values);
            const total = parseInt(countResult.rows[0].count, 10);
            // Get paginated results
            const offset = (page - 1) * limit;
            const result = await getPool().query(`SELECT * FROM ${tenantsTableFull}
         ${whereClause}
        ...
        async getTenantsForUser(userId) {
            const result = await getPool().query(`SELECT t.*, m.role as user_role, m.*
        ...
        async getTenantForUser(tenantId, userId) {
         ...
         FROM ${tenantsTableFull} t
         JOIN ${membershipsTableFull} m ON t.id = m.tenant_id
         WHERE t.id = $1 AND m.user_id = $2`, [tenantId, userId]);
            if (!result.rows[0])
                return null;
        ...
        async addMember(input) {
            ...
            const result = await getPool().query(`INSERT INTO ${membershipsTableFull} (tenant_id, user_id, role)
         VALUES ($1, $2, $3)
        ...
        async updateMember(tenantId, userId, input) {
            ...
            const result = await getPool().query(`UPDATE ${membershipsTableFull}
         SET role = $1
         WHERE tenant_id = $2 AND user_id = $3
         RETURNING *`, [role, tenantId, userId]);
            return result.rows[0] || null;
        },
        async removeMember(tenantId, userId) {
            const result = await getPool().query(`DELETE FROM ${membershipsTableFull}
         WHERE tenant_id = $1 AND user_id = $2`, [tenantId, userId]);
            return (result.rowCount ?? 0) > 0;
        },
        async getMembers(tenantId) {
            const result = await getPool().query(`SELECT * FROM ${membershipsTableFull}
         WHERE tenant_id = $1
         ORDER BY joined_at ASC`, [tenantId]);
            return result.rows;
        },
