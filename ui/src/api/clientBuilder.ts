...
export async function buildClientFromManifest<T = any>(
  ...
  if (!response.ok) {
    throw new Error(
      `Failed to fetch client manifest: ${response.status} ${response.statusText}`
    );
  }

  const manifest: ClientManifest = await response.json();
  ...
  for (const [key, route] of Object.entries(manifest.routes)) {
    ...
    client[camelNamespace][method] = createMethodFunction(
      baseUrl,
      route.method,
      route.path
    );
