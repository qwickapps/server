    ...
    <Box>
      <Box sx={{ mb: 3, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <Box>
          ...
          <Typography variant="body2" sx={{ color: 'var(--theme-text-secondary)', mt: 0.5 }}>
            Manage your user preferences as JSON
          </Typography>
        </Box>
        <Box sx={{ display: 'flex', gap: 1 }}>
          <Chip
            label={`${charCount.toLocaleString()} / ${MAX_PREFERENCES_SIZE.toLocaleString()} bytes`}
            size="small"
            color={sizePercent > 90 ? 'error' : sizePercent > 75 ? 'warning' : 'default'}
          />
        </Box>
      </Box>
      ...
      <Card sx={{ bgcolor: 'var(--theme-surface)', mb: 2 }}>
        <CardContent>
          ...
          <TextField
            ...
            error={!!jsonError}
            helperText={jsonError || `Edit your preferences as JSON. Max ${MAX_PREFERENCES_SIZE.toLocaleString()} bytes, max ${MAX_NESTING_DEPTH} levels deep.`}
            sx={{
              '& .MuiInputBase-root': {
      ...
      <Box sx={{ display: 'flex', gap: 2, justifyContent: 'flex-end' }}>
        ...
        <Button
          variant="contained"
          onClick={handleSave}
          disabled={!!jsonError || !hasChanges || saving}
          loading={saving}
        >
          Save Preferences
        </Button>
