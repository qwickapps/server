...
- **Plugins**: Fix module loading issue when importing from `@qwickapps/server/plugins`
  - Removed UI component exports from all plugin index files (13 plugins affected)
  - UI components now only exported from main package index `@qwickapps/server`
  - Prevents `ERR_PACKAGE_PATH_NOT_EXPORTED` errors when loading plugins without UI
  - Affected plugins: api-keys, auth, bans, devices, entitlements, notifications, parental, preferences, profiles, qwickbrain, rate-limit, subscriptions, usage
- **Notifications Plugin**: Fix TypeScript error with partial config
  - Create full config object with required `channels` field before passing to NotificationsManager
  - Fixes: `Type 'string[] | undefined' is not assignable to type 'string[]'`
