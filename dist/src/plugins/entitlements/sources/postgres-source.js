 ...
 * ```
 */
export function postgresEntitlementSource(config) {
    const { pool: poolOrFn, tableName = 'user_entitlements', definitionsTable = 'entitlement_definitions', schema = 'public', autoCreateTables = true, } = config;
    ...
    return {
      ...
      `);
        ...
        async getEntitlements(identifier) {
            ...
            const result = await getPool().query(`SELECT entitlement FROM ${entitlementsTable}
         WHERE LOWER(email) = $1
         AND (expires_at IS NULL OR expires_at > NOW())
         ORDER BY entitlement`, [email]);
            return result.rows.map((row) => row.entitlement);
        },
        async getAllAvailable() {
            const result = await getPool().query(`SELECT id, name, category, description FROM ${defsTable}
         ORDER BY category NULLS LAST, name`);
            return result.rows;
        },
        async getUsersWithEntitlement(entitlement, options = {}) {
            const { limit = 50, offset = 0 } = options;
            // Get total count
            const countResult = await getPool().query(`SELECT COUNT(DISTINCT email) FROM ${entitlementsTable}
         WHERE entitlement = $1
         AND (expires_at IS NULL OR expires_at > NOW())`, [entitlement]);
            const total = parseInt(countResult.rows[0].count, 10);
            // Get emails
            const result = await getPool().query(`SELECT DISTINCT email FROM ${entitlementsTable}
         WHERE entitlement = $1
         AND (expires_at IS NULL OR expires_at > NOW())
         ORDER BY email
         LIMIT $2 OFFSET $3`, [entitlement, limit, offset]);
            return {
                emails: result.rows.map((row) => row.email),
                total,
            };
        },
        async addEntitlement(identifier, entitlement, grantedBy) {
            const email = identifier.toLowerCase();
            // Use ON CONFLICT to handle duplicates (update granted_at if re-granting)
            await getPool().query(`INSERT INTO ${entitlementsTable} (email, entitlement, granted_by)
         VALUES ($1, $2, $3)
         ON CONFLICT (email, entitlement) DO UPDATE SET
           granted_at = NOW(),
           granted_by = EXCLUDED.granted_by,
           expires_at = NULL`, [email, entitlement, grantedBy || 'system']);
            // Auto-create definition if it doesn't exist
            await getPool().query(`INSERT INTO ${defsTable} (name)
         VALUES ($1)
         ON CONFLICT (name) DO NOTHING`, [entitlement]);
        },
        async removeEntitlement(identifier, entitlement) {
            const email = identifier.toLowerCase();
            await getPool().query(`DELETE FROM ${entitlementsTable}
         WHERE LOWER(email) = $1 AND entitlement = $2`, [email, entitlement]);
        },
        async setEntitlements(identifier, entitlements) {
            const email = identifier.toLowerCase();
            const pool = getPool();
            // Start a transaction
            ...
            try {
                ...
                await pool.query(`DELETE FROM ${entitlementsTable} WHERE LOWER(email) = $1`, [email]);
                // Insert new entitlements
                if (entitlements.length > 0) {
                    const values = entitlements
                        .map((_, i) => `($1, $${i + 2}, 'system')`)
                        .join(', ');
                    await pool.query(`INSERT INTO ${entitlementsTable} (email, entitlement, granted_by)
             VALUES ${values}`, [email, ...entitlements]);
                    // Auto-create definitions for any new entitlements
                    for (const ent of entitlements) {
                        await pool.query(`INSERT INTO ${defsTable} (name)
               VALUES ($1)
               ON CONFLICT (name) DO NOTHING`, [ent]);
                    }
                }
...
//# sourceMappingURL=postgres-source.js.map