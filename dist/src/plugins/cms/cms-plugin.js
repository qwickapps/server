 ...
 */
export function createCMSPlugin(config) {
    return {
        id: 'cms',
        name: 'CMS Plugin',
        version: '1.0.0',
            ...
            registry.addRoute({
                ...
                handler: async (_req, res) => {
                    try {
                        ...
                        logger.error('Failed to check CMS status:', error);
                        res.status(503).json({
                            status: 'down',
                            url: config.serviceUrl,
            ...
            registry.addRoute({
                ...
                handler: async (_req, res) => {
                    try {
                        ...
                        const seedFiles = files.filter(f => f.startsWith('seed-') && (f.endsWith('.mjs') || f.endsWith('.js')));
            ...
            registry.addRoute({
                ...
                handler: async (req, res) => {
                    ...
                    try {
                        ...
                        logger.error(`Failed to execute seed ${seedName}:`, error);
                        res.status(500).json({
                            success: false,
                            seedName,
                            error: error instanceof Error ? error.message : 'Unknown error',
...
//# sourceMappingURL=cms-plugin.js.map