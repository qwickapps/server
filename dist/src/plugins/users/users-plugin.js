    ...
    return {
        ...
        async onStart(_pluginConfig, registry) {
            ...
            if (config.api?.crud !== false) {
                ...
                WHERE id = $3`, [token, expiresAt, user.id]);
                            ...
                            res.status(201).json({
                                user: { ...user, status: 'invited', invitation_token: token, invitation_expires_at: expiresAt },
                                token,
                                inviteLink,
                    ...
                    handler: async (_req, res) => {
                        try {
                            ...
                            const stats = {
                                ...
                                recentSignups: allUsers.users.filter((u) => new Date(u.created_at).getTime() > sevenDaysAgo).length,
                            };
                            res.json(stats);
                        }
                        catch (error) {
 ...
 */
export async function getUserByIdentifier(identifiers) {
    if (!currentStore) {
        throw new Error('Users plugin not initialized');
    }
 ...
 */
export async function linkUserIdentifiers(userId, identifiers) {
    if (!currentStore) {
        throw new Error('Users plugin not initialized');
    }
