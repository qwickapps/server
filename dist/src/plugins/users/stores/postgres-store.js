 ...
 * ```
 */
export function postgresUserStore(config) {
    const { pool: poolOrFn, usersTable = 'users', schema = 'public', autoCreateTables = true, } = config;
    ...
    return {
        ...
        `);
                ...
                const statusCheck = await pool.query(`SELECT 1 FROM information_schema.columns
           WHERE table_schema = $1 AND table_name = $2 AND column_name = 'status'`, [schema, usersTable]);
                ...
                const tokenCheck = await pool.query(`SELECT 1 FROM information_schema.columns
           WHERE table_schema = $1 AND table_name = $2 AND column_name = 'invitation_token'`, [schema, usersTable]);
                ...
                const expiresCheck = await pool.query(`SELECT 1 FROM information_schema.columns
           WHERE table_schema = $1 AND table_name = $2 AND column_name = 'invitation_expires_at'`, [schema, usersTable]);
        ...
        async getByExternalId(externalId, provider) {
            const result = await getPool().query(`SELECT * FROM ${usersTableFull} WHERE external_id = $1 AND provider = $2`, [externalId, provider]);
            return result.rows[0] || null;
        },
        async getByIdentifier(identifiers) {
            // Validate that at least one identifier is provided
            const hasIdentifier = identifiers.email ||
            ...
            if (identifiers.email) {
                const result = await getPool().query(`SELECT * FROM ${usersTableFull} WHERE LOWER(email) = LOWER($1)`, [identifiers.email]);
                if (result.rows[0])
                    return result.rows[0];
            }
            // Priority 2: Auth0 user ID (stored in metadata.identifiers.auth0_user_id)
            if (identifiers.auth0_user_id) {
                const result = await getPool().query(`SELECT * FROM ${usersTableFull}
           WHERE metadata->'identifiers'->>'auth0_user_id' = $1`, [identifiers.auth0_user_id]);
                if (result.rows[0])
                    return result.rows[0];
                // Also check legacy external_id field for backwards compatibility
                const legacyResult = await getPool().query(`SELECT * FROM ${usersTableFull} WHERE external_id = $1`, [identifiers.auth0_user_id]);
            ...
            if (identifiers.wp_user_id !== undefined) {
                const result = await getPool().query(`SELECT * FROM ${usersTableFull}
           WHERE (metadata->'identifiers'->>'wp_user_id')::int = $1`, [identifiers.wp_user_id]);
                if (result.rows[0])
                    return result.rows[0];
            }
            ...
            if (identifiers.keap_contact_id !== undefined) {
                const result = await getPool().query(`SELECT * FROM ${usersTableFull}
           WHERE (metadata->'identifiers'->>'keap_contact_id')::int = $1`, [identifiers.keap_contact_id]);
                if (result.rows[0])
                    return result.rows[0];
            }
            return null;
        },
        async linkIdentifiers(userId, identifiers) {
            // Build the identifiers object to merge
            const identifiersToMerge = {};
            if (identifiers.wp_user_id !== undefined) {
            ...
            await getPool().query(`UPDATE ${usersTableFull}
         ...
         updated_at = NOW()
         WHERE id = $2`, [JSON.stringify(identifiersToMerge), userId]);
        },
        async create(input) {
            const result = await getPool().query(`INSERT INTO ${usersTableFull} (email, name, external_id, provider, picture, metadata)
         VALUES ($1, $2, $3, $4, $5, $6)
         RETURNING *`, [
        ...
        async update(id, input) {
            ...
            values.push(id);
            const result = await getPool().query(`UPDATE ${usersTableFull} SET ${updates.join(', ')} WHERE id = $${paramIndex} RETURNING *`, values);
            return result.rows[0] || null;
        },
        async delete(id) {
        ...
        async search(params) {
            const { query, provider, status, page = 1, limit = 20, sortBy = 'created_at', sortOrder = 'desc', } = params;
            const conditions = [];
            const values = [];
            let paramIndex = 1;
            if (query) {
            ...
            // Get total count
            const countResult = await getPool().query(`SELECT COUNT(*) FROM ${usersTableFull} ${whereClause}`, values);
            const total = parseInt(countResult.rows[0].count, 10);
            // Get users
            const result = await getPool().query(`SELECT * FROM ${usersTableFull} ${whereClause}
        ...
        async getByInvitationToken(token) {
            const result = await getPool().query(`SELECT * FROM ${usersTableFull} WHERE invitation_token = $1 AND invitation_expires_at > NOW()`, [token]);
            return result.rows[0] || null;
        },
        async acceptInvitation(token) {
            const result = await getPool().query(`UPDATE ${usersTableFull}
         SET status = 'active',
